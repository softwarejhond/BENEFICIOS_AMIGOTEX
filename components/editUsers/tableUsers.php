...
function getSedesAsesor($username, $conn) {
    ...
    if ($result->num_rows > 0) {
        ...
        return $row['sede1'] . (!empty($row['sede2']) ? ', ' . $row['sede2'] : '');
    }
...
<?php foreach ($users as $user) { 
    ...
    if ($user['rol'] == 3) {
        ...
        $stmtSedes->bind_param("i", $user['username']);
        $stmtSedes->execute();
        $resultSedes = $stmtSedes->get_result();
        if ($resultSedes->num_rows > 0) {
            $rowSedes = $resultSedes->fetch_assoc();
            $sede1 = $rowSedes['sede1'];
            $sede2 = $rowSedes['sede2'];
        }
        $stmtSedes->close();
    }
?>
